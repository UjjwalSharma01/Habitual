...
export default function HeatMap({ data }) {
  ...
  return (
    <div className="relative">
      {/* Month labels */}
      <div className="flex mb-1 text-xs text-gray-500 pl-10">
      ...
      <div className="flex">
        ...
        <div className="flex flex-col mr-2 text-xs justify-around text-right">
          {days.map((day, index) => (
            <div key={index} className="h-[11px] text-gray-500">{day}</div>
          ))}
        </div>
      ...
      {showTooltip && (
        <div 
          className="absolute z-10 px-3 py-2 text-sm text-white bg-gray-800 rounded-md"
          style={{ 
